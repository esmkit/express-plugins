...
var generateHTML = (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle, _htmlTplString, _jsTplString) => {
  var isExplorer=>
var customJs
var customJsStr
var swaggerUrls
...
var setup = (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle) => {
  var html = HTML(swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTit => le, htmlTplString, jsTplString)
  return (req, res) => {
    if (rrDoc) {=>
...
var swaggerInitFunction = (swaggerDoc, opts) => {
  var swaggerInitFile = js.toString().replace=> ('<% swaggerOptions %>', stringify(opts))
return (req, res, next) => {
  if (treq.url).endsWith=> ('/package.json')) {
    res.sendStatus(404)
  } else if (trimQuery(req.url).endsWith('/swagger-ui-init.js')) {
    if (req.swaggerDoc) {
...
var serveFiles = (swaggerDoc, opts) => {
  opts = opts || =>
var initOptions = {
  swaggerDoc: swaggerDoc || undefined,
...
var stringify = (obj, prop) => {
  var placeholdeFUNCTIONPLAC=> EHOLDER____'
var fns = []
var json = JSON.stringify(obj, (key, value) => {
  if (typeof value === 'functio=> 
      fns.push(value)
  return placeholder
}
    return value
